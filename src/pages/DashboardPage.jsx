...
function StatCard({ label, value, sub, color = "var(--accent)" }) {
  return (
    <div style={{
      ...
      borderRadius: "12px", padding: "1.25rem 1.5rem", position: "relative", overflow: "hidden",
    }}>
      <div style={{
  ...
  return (
    <div style={{ marginBottom: "0.75rem" }}>
      ...
      <div style={{ background: "var(--bg3)", borderRadius: "99px", height: "6px" }}>
        <div style={{ width: `${pct}%`, background: color, height: "6px", borderRadius: "99px" }} />
      </div>
    </div>
  );
}
...
function DashboardContent() {
  ...
  const toolCounts = logs.reduce((acc, l) => { acc[l.tool_id] = (acc[l.tool_id] ?? 0) + 1; return acc; }, {});
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--muted)", fontSize: "0.9rem" }}>
          ...
          {!import.meta.env.VITE_SUPABASE_URL && (
            <span style={{ color: "var(--amber)" }}>⚠️ Demo data — add VITE_SUPABASE_URL to go live.</span>
          )}
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "1.5rem" }}>
          ...
          {Object.entries(toolCounts).sort((a, b) => b[1] - a[1]).map(([tool, count]) => (
            <MiniBar key={tool} label={`${TOOL_ICONS[tool] ?? "🔧"} ${tool}`} value={count} max={maxCalls} color={TOOL_COLORS[tool] ?? "var(--accent)"} />
          ))}
        </div>
            ...
            {loading ? <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>Loading…</p> : (
              logs.slice(0, 20).map((log, i) => (
                <div key={log.id ?? i} style={{
                  ...
                  padding: "0.6rem 0.75rem", background: "var(--bg2)", borderRadius: "8px", fontSize: "0.8rem",
                }}>
